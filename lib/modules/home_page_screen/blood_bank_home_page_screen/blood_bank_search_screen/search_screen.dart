...
class BloodBankSearchScreen extends StatefulWidget {
  const BloodBankSearchScreen({super.key});

  @override
  State<BloodBankSearchScreen> createState() => _BloodBankSearchScreenState();
}

class _BloodBankSearchScreenState extends State<BloodBankSearchScreen> {
  void initState(){
  super.initState();
 /* try{
    DioHelper.getDataWithoutBody(url: 'bloodBank/lists', header: {
      "authentication": userAndTokenFromApiLoginPost!["token"]
    },param: {"bloodBankID":userAndTokenFromApiLoginPost!["bloodBankID"]}).then((val){print ('from search ${val.data}');
   setState(() {
  ...
  try{
    DioHelper.getDataWithoutBody(url: 'hospital/lists', header: {
      "authentication": userAndTokenFromApiLoginPost!["token"]
    },param: {"bloodBankID":userAndTokenFromApiLoginPost!["bloodBankID"]}).then((val){print ('from initState in search ${val.data}');
setState(() {
  hospitalListFromApi=val.data;
