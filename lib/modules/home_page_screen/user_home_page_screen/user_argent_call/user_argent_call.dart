...
class UserArgentCall extends StatefulWidget {
  const UserArgentCall({super.key});

  @override
  State<UserArgentCall> createState() => _UserArgentCallState();
}

class _UserArgentCallState extends State<UserArgentCall> {
  void initState(){
    super.initState();
    try{
      DioHelper.getDataWithoutBody(url: 'argentCall', header: {
        "authentication": userAndTokenFromApiLoginPost!["token"]
      },param: {"userID":userAndTokenFromApiLoginPost!["userID"]}).then((val){print ('from argentCall ${val.data}');
      setState(() {
        argentCall=val.data;
      });
      ;});
          ...
          children: [
            ...
            ListView.builder(
              ...
              return Card(child: Column(children: [
                Text('إسم المستشفى:  ${argentCall[index]["hospital"]["name"]}', style: TextStyle(fontSize: 17.0, color: mainColor)),
                SizedBox(
                  height: 5,
                ),
